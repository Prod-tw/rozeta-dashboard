...
test('timeline meeting geometry places a visible meeting in the corresponding window segment', () => {
	const window = timelineWindow(new Date('2026-08-08T10:15:00+08:00'))
	const start = new Date('2026-08-08T09:30:00+08:00')
	const end = new Date('2026-08-08T10:00:00+08:00')
	const top = timelinePositionPercent(start, window)
	const bottom = timelinePositionPercent(end, window)
	assert.equal(top, 12.5)
	assert.equal(bottom, 37.5)
	assert.equal(bottom - top, 25)
})
